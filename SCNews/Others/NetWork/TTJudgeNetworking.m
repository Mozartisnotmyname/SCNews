...
+ (BOOL)judge {
    if ([[SCReachability reachabilityForInternetConnection] currentReachabilityStatus] == NotReachable) {
        return NO;
    }
    return YES;
}
...
+(NetworkingType)currentNetworkingType {
    SCReachability *reachablility =  [SCReachability reachabilityWithHostName:@"www.baidu.com"];
